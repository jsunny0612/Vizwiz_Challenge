...
#######################################################################################################################
def create_submit_file(model: torch.nn.Module,
                 data_loader: torch.utils.data.DataLoader, mask : list,
                 device: torch.device = None):
    if device is None:
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    ...
    with torch.no_grad():
        for i, data in enumerate(tqdm(data_loader)):
            imgs, labels = data[0], data[1]
            output = model([img.to(device) for img in imgs]) if isinstance(imgs, list) else model(imgs.to(device))
            predictions = output[:,mask].argmax(1)

            for j in range(len(predictions)):
                data_num = i * batch_size + j
                results[data_loader.dataset.samples[data_num][0].split("/")[-1]] = mask[predictions[j]]

    return results
########################################################################################################################

def create_submit_file_for_new_idea_lame\
...
############################################################################################################ func start
    ...
    with torch.no_grad():
        for i, data in enumerate(tqdm(data_loader)):
            imgs, labels = data[0], data[1]
            output = model([img.to(device) for img in imgs]) if isinstance(imgs, list) else model(imgs.to(device), epoch, i)
...
def create_submit_file_for_new_idea\
                 (model: torch.nn.Module,
                 data_loader: torch.utils.data.DataLoader,
                 mask : list,
                 device: torch.device = None,
...
############################################################################################################ func start
    ...
    mask.sort()
    class_mask = torch.zeros(1000, device=device)
    class_mask[mask] = 1

    with torch.no_grad():
        for i, data in enumerate(tqdm(data_loader)):
            imgs, labels = data[0], data[1]
            predictions = model([img.to(device) for img in imgs], epoch, i, class_mask, mask) if isinstance(imgs, list) else model(imgs.to(device), epoch, i, class_mask, mask)

            for j in range(len(predictions)):
                data_num = i * batch_size + j
                image_name = data_loader.dataset.samples[data_num][0].split("/")[-1]
                if image_name in image_list:
                    results[image_name] = mask[predictions[j].argmax()]
...
def create_submit_file_for_LNL\
                 (model: torch.nn.Module,
                 data_loader: torch.utils.data.DataLoader,
                 mask : list,
                 device: torch.device = None,
...
######################################################################################################################## func start
def create_shot_predict(loader, model):
    start_test = True
    with torch.no_grad():
        iter_test = iter(loader)
        for i in range(len(loader)):
    ...
    return all_output, loader.dataset.dataset.samples
