...
class Parallel_psedo(nn.Module):
######################################################################################################################## class start
...
######################################################################################################################## class end
    ...
    def forward_and_adapt(self, x, optimizer, optimizer_teacher, epoch, iter, class_mask, class_number):
        ...
        # forward
        outputs = (self.model(x)*class_mask)[:,class_number]
        ...
        student_loss = (0.5 * loss_ce) + (0.5 * loss_div)
        student_loss.backward()
        optimizer.step()
        optimizer.zero_grad()
        ################################################################################################################ loss for student end
        ...
        with open(self.output_dir + f'/parallel_psedo_{str(self.model)[:11]}.txt', 'a') as f:
            f.write(f'mask {int(label_mask.sum())} loss_div: {loss_div} loss_ce: {loss_ce} student_loss: {student_loss} teacher_loss: {teacher_loss} \n')
    ...
    def configure_model(model):
        ...
        # for stage in model.module.stages:
        #     for block in stage.blocks:
        #         block.parallel.weight, block.parallel.bias = deepcopy(block.conv_dw.weight), deepcopy(block.conv_dw.bias)
