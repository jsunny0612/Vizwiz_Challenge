...
class SAR_para(nn.Module):
    ...
    """
    def __init__(self, model, lr, batch_size, num_classes, episodic=False, reset_constant=0.2, steps=1, e_margin=0.4*math.log(10000), adaptation_type ='otta'):
        super().__init__()
    ...
    def forward(self, x):
        ...
        if self.adaptation_type == 'ttba':
            ...
            for _ in range(self.steps):
                _, ema = forward_and_adapt_sar_ttba(x, self.model, self.optimizer, self.margin_e0,
        ...
        ################################################################################################ if otta start
            ...
            for _ in range(self.steps):
                outputs, ema, reset_flag = forward_and_adapt_sar(x, self.model, self.optimizer, self.margin_e0,
                                                                 self.reset_constant_em, self.ema)
                # if reset_flag:
                #     self.reset()
...
############################################################################################################################################################### forward end
    ...
    def prepare_SAR_model_and_optimizer(model, lr, batch_size):
        model = SAR_para.configure_model(model)
...
def forward_and_adapt_sar(x, model, optimizer, margin, reset_constant, ema):
    ...
    # filter_ids_1 = torch.where((entropys < margin) & (entropys > 0.5))
    ...
    # filter_ids_2 = torch.where((entropys2 < margin) & (entropys2 > 0.5))  # here filtering reliable samples again, since model weights have been changed to \Theta+\hat{\epsilon(\Theta)}
    ...
    ####################################################################################################################
    with open(f'/mnt/HDD2/HW_2/Benchmark-TTA/output/test-time-evaluation/debug/{str(model)[:11]}.txt', 'a') as f:
        f.write(f'loss1: {loss} loss: {loss_second} \n')
    ####################################################################################################################
...
@torch.enable_grad()  # ensure grads in possible no grad context for testing
def forward_and_adapt_sar_ttba(x, model, optimizer, margin, reset_constant, ema):
    """Forward and adapt model input data.
    Measure entropy of the model prediction, take gradients, and update params.
    """
    optimizer.zero_grad()
